 ...
 *
 * 本檔完整示範兩件事：
 *   1. 為什麼「未同步」會發生競爭條件（超賣、餘 -1、重複售出）。
 *   2. 用 synchronized 同步區塊修正，確保總售出張數正好 100。
 *
 * 執行方式：
 *   javac TicketSystem.java
 *   java  TicketSystem            → 執行「安全版」（預設）
 *   java  TicketSystem unsafe     → 執行「危險版」，觀察競爭條件
 */
public class TicketSystem {

    /* ============================================================
     * 售票任務：實作 Runnable，讓多條執行緒「共用同一個 job 物件」，
     * 因此票庫 tickets 與鎖 lock 都宣告為 static（全類別共享一份）。
     * ============================================================ */
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        System.out.println(safe
                ? "安全版：剩餘應為 0，且沒有超賣。"
                : "危險版：可能出現「餘 -1」「同一張票賣兩次」等錯誤，這就是競爭條件。");
    }
}
